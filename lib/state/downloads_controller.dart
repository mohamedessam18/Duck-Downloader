...
class DuckDownloadsController extends ChangeNotifier
  ...
  Future<void> _extractUrlOrBatch(String url) async {
    ...
    if (lines.length > 1) {
      ...
      // ── TikTok: always use Cobalt — no login required ──────────────────────
      if (CobaltService.isTikTok(cleanUrl)) {
        flow = DuckFlow.extracting;
        status = 'Fetching TikTok info...';
        notifyListeners();

        // Photo slide / multi-image post
        final pickerUrls = await CobaltService.getPickerUrls(url: cleanUrl);
        if (pickerUrls.isNotEmpty) {
          final items = <PlaylistItem>[];
          for (final u in pickerUrls) {
            items.add(PlaylistItem(url: u, title: 'TikTok Photo', isVideo: false));
          }
          batchTitle = 'TikTok Photos';
          batchPlatform = 'TikTok';
          batchItems = items;
          selectedType = DownloadType.image;
          quality = 'Original';
          flow = DuckFlow.ready;
          status = 'Tap download to save';
          return;
        }
        ...
        if (cobaltUrl != null) {
          ...
          metadata = MediaMetadata(
            ...
            platform: 'TikTok',
            duration: null,
            qualities: [const FormatInfo(id: '1080p', label: '1080p', ext: 'mp4')],
            audioFormats: [const FormatInfo(id: 'audio', label: 'Audio (MP3)', ext: 'mp3')],
          );
          selectedType = DownloadType.video;
          quality = '1080p';
          flow = DuckFlow.ready;
      ...
      // ── Twitter/X, Reddit, SoundCloud: Cobalt-first — no login required ────
      if (CobaltService.isSupported(cleanUrl) &&
          !cleanUrl.contains('instagram.com') &&
          !cleanUrl.contains('facebook.com') &&
          !cleanUrl.contains('fb.watch') &&
          !CobaltService.isTikTok(cleanUrl)) {
        flow = DuckFlow.extracting;
        status = 'Fetching media info...';
        notifyListeners();
        ...
        if (cobaltUrl != null) {
          final platform = cleanUrl.contains('twitter.com') || cleanUrl.contains('x.com')
              ? 'Twitter/X'
              : cleanUrl.contains('reddit.com')
                  ? 'Reddit'
                  : cleanUrl.contains('soundcloud.com')
                      ? 'SoundCloud'
                      : 'Social';
          final isAudio = cleanUrl.contains('soundcloud.com');
          final fakeTitle = '${platform}_${DateTime.now().millisecondsSinceEpoch}';
          metadata = MediaMetadata(
            url: cleanUrl,
            title: fakeTitle,
            thumbnail: null,
            platform: platform,
            duration: null,
            qualities: [const FormatInfo(id: 'best', label: 'Best', ext: 'mp4')],
            audioFormats: [const FormatInfo(id: 'audio', label: 'Audio (MP3)', ext: 'mp3')],
          );
          selectedType = isAudio ? DownloadType.audio : DownloadType.video;
          quality = 'Best';
          flow = DuckFlow.ready;
          status = isAudio ? 'Tap download to save audio' : 'Choose video or audio';
          return;
        }
        // Fall through to server if Cobalt fails
      }
